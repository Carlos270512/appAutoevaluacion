...
<div id="headerr">
      <ul class="nav">
        ...
        <li><h4 style="color: white; font-size: 20px; margin-top: 20px; margin-left: 10px"><b>Coevaluación de Docentes</b></h4></li>
     </ul>
</div>
</header>
 ...
 <body>
  ...
  <div id="inicio_admin" class="menu-collapsed">
       ...
       <div id="profile">
            ...
            <div id="name"><span><b>Bienvenido:</b> <br><h4 style="color: white; font-size: 16px"><b><span class="fa fa-user icon-menu" style="margin-right: 5px;margin-top: 10px;"></span><?php echo $_SESSION['usu']['usuario'] ?></b></h4></span></div>
       </div>

    <div id="menu-items">
    <div class="item">
    <ul>
            <li><a href="../coordinador.php">
...
?>
    ...
    <div class="logo-title2" style="margin-top: 43px" >

<form method="post" action="reporte2.php" style="display: block;" >
        
<form action="buscar.php" method="get">

  <label for=fecha_inicio style="font-size:14px;position: relative; top:-35;left: 5px;"><b>FECHA DE INICIO:</b> </label>
  <input type="date"  readonly="" style="position: relative;top:-2px; left: -127px;" class="user"  value="<?php echo $fecha_inicio; ?>" id="fecha_inicio" name="fecha_inicio" >
  <label for=fecha_final style="font-size:14px;position: relative; top:-35;left: -112px; "><b>FECHA FIN:</b> </label>
  <input type="date" readonly="" style="position: relative; top:-2;left: -200px; " class="user3" value="<?php echo $fecha_final; ?>" id="fecha_final" name="fecha_final" >

  <button class="button2" type="submit" style="position: relative; top:0;left: -165px; " name="generar_reporte"><i class="fa fa-file-excel-o"></i> Generar Excel Periódico</button>
</form>
 <a href="convertidor.php" class="button3" style="position: relative;top:-10px; left: -155px;"><i class="fa fa-files-o"></i> Ver Todos los Reportes</a>
</div>
    ...
    <tr class="head">
      ...
      <td style="font-size: 12px">NIVEL</td>
      <td style="font-size: 12px">CARRERA</td>
      <td style="font-size: 12px">MODALIDAD</td>
      <td style="font-size: 12px">JORNADA</td>
      <td style="font-size: 12px">PERÍODO</td>
    ...
    <tr></tr>
      ...
      <?php
      $conexion =new mysqli("localhost","desaroll_s0ftar3vN22","fsTbroeeSuAC-2022","desaroll_istvn_sistema_radio"); 
  
 $sentencia="SELECT c.id,c.fecha,c.docente,c.asignatura,c.codigo,c.nivel,c.carrera,c.modalidad,c.jornada,p.periodo FROM coevaluacion c, login l, periodolectivo p WHERE id_coordinador='".$_SESSION['usu']['id']."' AND c.docente=l.nombre AND c.periodo=p.id AND fecha BETWEEN '$fecha_inicio' AND '$fecha_final' ORDER BY fecha";
      $resultado= $conexion->query($sentencia) or die (mysqli_error($conexion));
      while($fila=$resultado->fetch_assoc())
      {
        $newDate = date("d/m/Y", strtotime($fila['fecha']));
        echo "<tr>";
        echo "<td width='90px'>"; echo $newDate; echo "</td>";
      
        echo "<td style='font-size: 12px;'>"; echo $fila['docente']; echo "</td>";
        echo "<td style='font-size: 12px;'>"; echo $fila['asignatura']; echo "</td>";
        echo "<td style='font-size: 12px;'>"; echo $fila['codigo']; echo "</td>";
        echo "<td style='font-size: 12px;'>"; echo $fila['nivel']; echo "</td>";
        echo "<td style='font-size: 12px;'>"; echo $fila['carrera']; echo "</td>";
        echo "<td style='font-size: 12px;'>"; echo $fila['modalidad']; echo "</td>";
        echo "<td style='font-size: 12px;'>"; echo $fila['jornada']; echo "</td>";
        echo "<td style='font-size: 12px;'>"; echo $fila['periodo']; echo "</td>";
              
        echo "<td><div align='center'><a href='Generar_PDF.php?id=".$fila['id']."' target='_blank'><button type='button' class='btn__del'><i class='icon3 fa fa-file-pdf-o'></i> Generar PDF</button></a></div></td>";
       
         /*echo "<td><a href='enviar/envia_presencial/enviar.php?id=".$fila['id']."'> <button type='button' class='btn__env'><i class='icon3 fa fa-envelope'></i> Enviar</button> </a></td>";*/

         
          echo "<td><div align='center'><a href='1modif_prod1.php?id=".$fila['id']."'> <button type='button' class='btn__upd'><i class='icon3 fa fa-edit'></i> Editar</button> </a></div></td>";
